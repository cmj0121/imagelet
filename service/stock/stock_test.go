...
func TestStripPylonSyntax(t *testing.T) {
	cases := []struct {
		...
		// `‌` is U+200C ZWNJ — invisible on every renderer, only
		// inserted when `&` is immediately followed by [A-Za-z_] (the
		// pylon Ref-parser trigger). Standalone `&` is unchanged.
		...
		{"ampersand_before_digit", "AT&T 5G", "AT&‌T 5G"},
