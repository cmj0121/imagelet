...
func TestMAPositionBar_ZeroInputReturnsEmpty(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			top, bar, caption := MAPositionBar(tc.ma10, tc.ma5, tc.price, 0, 30, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
			if top != "" || bar != "" || caption != "" {
...
func TestMAPositionBar_BarRowEqualsWidth(t *testing.T) {
	width := 41
	top, bar, _ := MAPositionBar(100, 102, 101, 99.5, width, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_PriceAtCenter(t *testing.T) {
	width := 31
	_, bar, _ := MAPositionBar(100.5, 99.5, 100, 0, width, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
	runes := []rune(bar)
	centerCol := width / 2
...
func TestMAPositionBar_TextMarkersPresent(t *testing.T) {
	...
	width := 60
	_, bar, _ := MAPositionBar(98.5, 101.5, 100, 0, width, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_M10WinsOverM5(t *testing.T) {
	...
	_, bar, _ := MAPositionBar(99, 99, 100, 0, 31, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
	if !strings.Contains(bar, maMarkerMA10) {
...
func TestMAPositionBar_SaturationRight(t *testing.T) {
	...
	price := 100.0
	width := 31
	_, bar, _ := MAPositionBar(ma10, ma5, price, 0, width, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
	runes := []rune(bar)
...
func TestMAPositionBar_WidthClamping(t *testing.T) {
	_, bar, _ := MAPositionBar(100, 100, 100, 0, 2, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
	if l := len([]rune(bar)); l != maBarMinWidth {
		t.Errorf("width=2 clamped to %d, want %d (%q)", l, maBarMinWidth, bar)
	}
	_, bar, _ = MAPositionBar(100, 100, 100, 0, 200, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_PrevCloseMarkerOnTop(t *testing.T) {
	...
	top, _, _ := MAPositionBar(99.5, 100.5, 100, prev, width, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_PrevCloseZeroOmitsMarker(t *testing.T) {
	top, _, _ := MAPositionBar(99.5, 100.5, 100, 0, 60, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
	if strings.Contains(top, string(ohlcMarkerPrev)) {
...
func TestMAPositionBar_CaptionFormat(t *testing.T) {
	...
	_, _, caption := MAPositionBar(100, 101, 102, 0, 60, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_CaptionDeathCross(t *testing.T) {
	_, _, caption := MAPositionBar(101, 100, 99, 0, 60, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
	if !strings.Contains(caption, "5↘10") {
		t.Errorf("caption missing 5↘10 trend token: %q", caption)
	}
}
...
func TestMAPositionBar_CaptionEqualMAs(t *testing.T) {
	_, _, caption := MAPositionBar(100, 100, 99, 0, 60, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_CaptionPriceBelowMA(t *testing.T) {
	...
	_, _, caption := MAPositionBar(100, 101, 98, 0, 60, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
...
// TestMAPositionBar_KeepsCAnchor pins that the MA bar always carries
// a `C` glyph at the center column — even when the OHLC bar above is
// suppressing its own C on the open-market path. The MA bar's `C` is
// purely positional (the OHLC bar's OCP row already conveys "no real
// close yet" via `C: -`), and without it the bar collapses to floating
// MA labels when both M5 / M10 cluster on one side of price.
func TestMAPositionBar_KeepsCAnchor(t *testing.T) {
	width := 31
	_, bar, _ := MAPositionBar(99, 101, 100, 99.5, width, SymmetricBand(priceBandScale), DefaultMALabels(), fmtPrice)
	runes := []rune(bar)
	centerCol := width / 2
	if runes[centerCol] != ohlcMarkerClose {
		t.Errorf("center col %d should carry the `C` anchor, got %q in %q",
			centerCol, string(runes[centerCol]), bar)
	}
	if !strings.Contains(bar, maMarkerMA10) {
		t.Errorf("M10 marker should still render: %q", bar)
	}
	if !strings.Contains(bar, maMarkerMA5) {
		t.Errorf("M5 marker should still render: %q", bar)
	}
}
