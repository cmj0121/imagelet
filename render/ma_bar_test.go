...
func TestMAPositionBar_ZeroInputReturnsEmpty(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			top, bar, caption := MAPositionBar(tc.ma10, tc.ma5, tc.price, 0, true, 30, priceBandScale, DefaultMALabels(), fmtPrice)
			if top != "" || bar != "" || caption != "" {
...
func TestMAPositionBar_BarRowEqualsWidth(t *testing.T) {
	width := 41
	top, bar, _ := MAPositionBar(100, 102, 101, 99.5, true, width, priceBandScale, DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_PriceAtCenter(t *testing.T) {
	width := 31
	_, bar, _ := MAPositionBar(100.5, 99.5, 100, 0, true, width, priceBandScale, DefaultMALabels(), fmtPrice)
	runes := []rune(bar)
	centerCol := width / 2
...
func TestMAPositionBar_TextMarkersPresent(t *testing.T) {
	...
	width := 60
	_, bar, _ := MAPositionBar(98.5, 101.5, 100, 0, true, width, priceBandScale, DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_M10WinsOverM5(t *testing.T) {
	...
	_, bar, _ := MAPositionBar(99, 99, 100, 0, true, 31, priceBandScale, DefaultMALabels(), fmtPrice)
	if !strings.Contains(bar, maMarkerMA10) {
...
func TestMAPositionBar_SaturationRight(t *testing.T) {
	...
	price := 100.0
	width := 31
	_, bar, _ := MAPositionBar(ma10, ma5, price, 0, true, width, priceBandScale, DefaultMALabels(), fmtPrice)
	runes := []rune(bar)
...
func TestMAPositionBar_WidthClamping(t *testing.T) {
	_, bar, _ := MAPositionBar(100, 100, 100, 0, true, 2, priceBandScale, DefaultMALabels(), fmtPrice)
	if l := len([]rune(bar)); l != maBarMinWidth {
		t.Errorf("width=2 clamped to %d, want %d (%q)", l, maBarMinWidth, bar)
	}
	_, bar, _ = MAPositionBar(100, 100, 100, 0, true, 200, priceBandScale, DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_PrevCloseMarkerOnTop(t *testing.T) {
	...
	top, _, _ := MAPositionBar(99.5, 100.5, 100, prev, true, width, priceBandScale, DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_PrevCloseZeroOmitsMarker(t *testing.T) {
	top, _, _ := MAPositionBar(99.5, 100.5, 100, 0, true, 60, priceBandScale, DefaultMALabels(), fmtPrice)
	if strings.Contains(top, string(ohlcMarkerPrev)) {
...
func TestMAPositionBar_CaptionFormat(t *testing.T) {
	...
	_, _, caption := MAPositionBar(100, 101, 102, 0, true, 60, priceBandScale, DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_CaptionDeathCross(t *testing.T) {
	_, _, caption := MAPositionBar(101, 100, 99, 0, true, 60, priceBandScale, DefaultMALabels(), fmtPrice)
	if !strings.Contains(caption, "5↘10") {
		t.Errorf("caption missing 5↘10 trend token: %q", caption)
	}
}
...
func TestMAPositionBar_CaptionEqualMAs(t *testing.T) {
	_, _, caption := MAPositionBar(100, 100, 99, 0, true, 60, priceBandScale, DefaultMALabels(), fmtPrice)
...
func TestMAPositionBar_CaptionPriceBelowMA(t *testing.T) {
	...
	_, _, caption := MAPositionBar(100, 101, 98, 0, true, 60, priceBandScale, DefaultMALabels(), fmtPrice)
...
// TestMAPositionBar_OpenSessionSuppressesC pins the open-market path:
// when closed=false the C glyph at the center column is suppressed
// (same rule as OHLCBar). The center column stays as wick `─`, M5 /
// M10 keep their offset positions, and the ▼ overlay on top still
// renders. Mirrors the OHLC suppression so a stacked OHLC + MA pair
// reads consistently while the session is open.
func TestMAPositionBar_OpenSessionSuppressesC(t *testing.T) {
	width := 31
	_, bar, _ := MAPositionBar(99, 101, 100, 99.5, false, width, priceBandScale, DefaultMALabels(), fmtPrice)
	runes := []rune(bar)
	centerCol := width / 2
	if runes[centerCol] != maBarBackground {
		t.Errorf("center col %d should be wick `─` when market is open, got %q in %q",
			centerCol, string(runes[centerCol]), bar)
	}
	if strings.ContainsRune(bar, ohlcMarkerClose) {
		t.Errorf("bar must not carry C glyph when market is open: %q", bar)
	}
	if !strings.Contains(bar, maMarkerMA10) {
		t.Errorf("M10 marker should still render when market is open: %q", bar)
	}
	if !strings.Contains(bar, maMarkerMA5) {
		t.Errorf("M5 marker should still render when market is open: %q", bar)
	}
}
